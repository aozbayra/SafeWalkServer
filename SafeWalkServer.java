...
public class SafeWalkServer implements Runnable {
	...
	private void serverShutdown() throws IOException {
		PrintWriter clientShutdown = new PrintWriter(socket.getOutputStream());
		clientShutdown.println("RESPONSE: success");
		clientShutdown.flush();

		for (Client i : clientList) {
			if (!i.socket.isClosed()) {
				PrintWriter pw = new PrintWriter(i.getOutputStream());
				pw.println("ERROR: connection reset");
				pw.flush();
				i.socket.close();
	...
	public int checkMatch() {
		...
		for (Client i : clientList) {
			if (i.from.equals(fromCurrent)) {
				if (i.to.equals(toCurrent) || i.to.equals("*")
						|| toCurrent.equals("*")) {
	...
	public void giveResponse(int matchIndex) throws IOException {
		...
		current.println("RESPONSE: " + pairedClient.name + ","
				+ pairedClient.from + "," + pairedClient.to + ","
				+ pairedClient.type);
		current.flush();
